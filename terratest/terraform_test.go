...
func TestTerraformBasicExampleNew(t *testing.T) {
	t.Parallel()
	//need change the value before run
	vswitchId := "tf-eip-slb-ecs-rds"
	...
	thisVswitchId := terraform.Output(t, terraformOptions, "vswitch_id")

	// Verify we're getting back the outputs we expect
	assert.Equal(t, thisVswitchId, vswitchId)
}
